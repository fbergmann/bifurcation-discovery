...
namespace BifurcationFrontend
{
    ...
    public class FrontendService
    {
        ...
        /// </summary>
        private static BifurcationForm m_oForm;
        ...
        public void updateStatus(ArrayList oList)
        {
            ...
            try
            {
                var nStatus = (int) oList[0];
                if (nStatus == 0)
                {
                    var nNumberOfSimulations = (int) oList[1];
                    m_oForm.updateNumberOfSimulations(nNumberOfSimulations);
                }
                else
                {
                    var nIteration = (int) oList[1];
                    //int nNumberOfSimulations;
                    var dFitness = (double) oList[2];
                    double[] dParameter = ((List<double>) oList[3]).ToArray();
                    var dEigenValueReal = (double) oList[4];
                    var dEigenValueComplex = (double) oList[5];
                    double[] dRealEigen = ((List<double>) oList[6]).ToArray();
                    double[] dComplexEigen = ((List<double>) oList[7]).ToArray();
                    m_oForm.update(nIteration, dFitness, dEigenValueReal, dEigenValueComplex, dParameter, dRealEigen,
                        dComplexEigen);
                }
